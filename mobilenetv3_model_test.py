...
class MobileNetV3(nn.Module):
    ...
    def regist_hook(self):
        ...
        def get(model, input, output):
            ...
            self.extraction.append(output.detach())

        for name, module in self._modules['features']._modules.items():
            if name in ['1', '3', '7', '11']:
                self._modules['features'][int(name)].conv._modules['9'].register_forward_hook(get)
        for name, module in self._modules['features']._modules.items():
            if name in ['0']:
                self._modules['features'][int(name)]._modules['3'].register_forward_hook(get)
